          ...
          onClick={() => {
            if (confirm('Tem certeza que deseja excluir este avaliador?')) {
              // Faz a requisição DELETE usando Inertia
                router.delete(route('avaliadores.destroy', id))
            }
          }}
        >
...
export default function Avaliadores({ areas }: { areas: Area[] }) {
  const { avaliadores } = useUsuarios();
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <ModalCriar open={openCriar} setOpen={setOpenCriar} dado={null} areas={areas}/>
      <ModalImportar
        open={openImportar}
      ...
      <div className="flex justify-between gap-2 m-2">
          <div>
            ...
            <p className="text-muted-foreground">
              Gerencie os avaliadores cadastrados no sistema.
            </p>
          </div>
